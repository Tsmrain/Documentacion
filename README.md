...
El sistema OpenBJJ es una aplicación web inteligente que actúa como tutor deportivo adaptativo y asistente cinemático. En esta primera línea base ejecutable, el software analiza videos monoculares en 2D correspondientes a ejecuciones individuales de técnicas (drills técnicos, sombra de BJJ o ejecuciones encuadradas del practicante principal), autodetectando la técnica mediante IA multimodal. El estimador local MediaPipe Pose aísla los 33 landmarks corporales del practicante evaluado, calculando métricas articulares en 3D en tiempo real y evaluando el movimiento contra la base de datos vectorial centralizada (RAG Vivo). La oclusión física masiva por contacto directo continuo entre dos oponentes en sparrings se documenta formalmente como una limitación cinemática monocular y alcance de evolución futura.
...
- **RF03: Agregar Fuente Opcional y RAG Personalizado:** El sistema debe permitir al Practicante agregar de manera opcional archivos PDF y lecciones de YouTube (mediante los endpoints `POST /api/rag/ingestar`, `GET /api/rag/fuentes` y `DELETE /api/rag/fuentes/:id` - CU02 / RD-03) para enriquecer el Vector DB de forma personalizada. Para las fuentes de YouTube, el Servidor Local realiza la extracción y vectorización de metadatos instruccionales estructurados (título, canal y descripción vía oEmbed) para construir chunks semánticos de 384 dimensiones en ChromaDB. Si el Practicante no provee ninguna fuente, el sistema no presentará fallos y entrará en "Modo Fallback Baseline".
...
5. El `RetrievalAugmentedController` intercepta el enlace de YouTube y recupera los metadatos instruccionales estructurados (título, canal y descripción de la lección) mediante el protocolo `oEmbed`.
...
4. La PWA utiliza la clase de Fabricación Pura `AntropometriaParser` para realizar la traducción y normalización aritmética asíncrona por ergonomía de tatami (RNF07), dividiendo la altura ingresada en centímetros entre 100 para entregar el valor estandarizado en metros (ej. `175 cm` a `1.75 m`). La entidad de dominio `Usuario` (Information Expert) encapsula la validación de sus propios límites antropométricos (altura: 0.50-2.50 m, peso: 30-250 kg), preservando la alta cohesión de los controladores de casos de uso.
...
La capa de persistencia relacional utiliza **PostgreSQL** administrado a través de **Prisma ORM** (v6+). El esquema relacional define 8 entidades (`Usuario`, `PerfilCompetencia`, `SesionEntrenamiento`, `AnalisisBiomecanico`, `ErrorBiomecanico`, `HistorialVisualizacion`, `RutaAprendizaje`, `FuenteConocimiento`). Para prevenir ciclos referenciales y bloqueos concurrentes en el motor relacional, la eliminación física en cascada (`onDelete: Cascade`) se estructuró bajo una precedencia estrictamente lineal (`Usuario` $\rightarrow$ `SesionEntrenamiento` $\rightarrow$ `AnalisisBiomecanico` $\rightarrow$ `ErrorBiomecanico`), garantizando la purga automática limpia de las 8 tablas sin registros huérfanos ni colisiones de llaves foráneas.
