...
La capa de persistencia relacional utiliza **PostgreSQL** administrado a través de **Prisma ORM** (v6+). El esquema relacional define 8 entidades (`Usuario`, `PerfilCompetencia`, `SesionEntrenamiento`, `AnalisisBiomecanico`, `ErrorBiomecanico`, `HistorialVisualizacion`, `RutaAprendizaje`, `FuenteConocimiento`). Para prevenir ciclos referenciales y bloqueos concurrentes en el motor relacional, la entidad `ErrorBiomecanico` depende existencialmente de forma lineal y exclusiva de `AnalisisBiomecanico` (`Usuario` $\rightarrow$ `SesionEntrenamiento` $\rightarrow$ `AnalisisBiomecanico` $\rightarrow$ `ErrorBiomecanico`), eliminando caminos múltiples de borrado en cascada (`onDelete: Cascade`) desde `Usuario` en PostgreSQL y Prisma. Por su parte, `PerfilCompetencia` lee el historial de errores mediante consultas de agregación lógica (JOINs indexados) sin acoplamientos físicos de cascada.
