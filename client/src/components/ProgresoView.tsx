...
export function ProgresoView({ usuarioId }: ProgresoViewProps) {
  ...
  if (error || !data) {
    return (
      <div className="glass-panel p-6 animate-fade-in" style={{ padding: '24px' }}>
        ...
        <p style={{ color: '#cbd5e1' }}>{error || "No se pudo recuperar la información del practicante."}</p>
  ...
  return (
    <div className="glass-panel p-6 animate-fade-in mb-6" style={{ padding: '24px' }}>
      <h2 style={{ marginTop: 0, color: '#818cf8', fontSize: '1.4rem' }}>Progreso y Tutoría Adaptativa (CU03)</h2>
      <p style={{ color: '#94a3b8', fontSize: '0.9rem' }}>
        Evolución técnica del practicante y recomendaciones pedagógicas adaptativas personalizadas.
      </p>
      ...
      {esAlertaAdaptativa && (
        <div style={{ padding: '16px', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: '8px', marginBottom: '24px' }}>
          <span style={{ fontSize: '0.75rem', color: '#f87171', textTransform: 'uppercase', fontWeight: 'bold', display: 'block', marginBottom: '4px' }}>
            Alerta de Fallo Recurrente (Más de 3 Fallos Consecutivos)
          </span>
          <p style={{ margin: '0 0 12px 0', color: '#fecaca', fontSize: '0.9rem', lineHeight: '1.4' }}>
            {data.mensajeAdaptativo || "Se ha detectado una desviación persistente en tus últimos sparrings. Se recomienda conmutar la estrategia a un drill de aislamiento."}
          </p>
