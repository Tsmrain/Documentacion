// ============================================================================
// OpenBJJ - PromptBuilder (Experto en Información GRASP)
// Centraliza y ensambla las reglas técnicas del BJJ para el LLM
// Incluye grounding RAG, métricas cinemáticas, y formato JSON estricto
// ============================================================================
...
export class PromptBuilder {
   ...
   * Fuerza respuesta JSON estricta para evitar alucinaciones.
   */
  buildPrompt(params: {
    ...
    // Contexto RAG (solo fuentes validadas, regla RD-03)
...
RESPONDE ÚNICAMENTE con el siguiente objeto JSON (sin texto adicional, sin markdown, sin bloques de código):
{
  ...
  /**
   * Calcula el porcentaje de ajuste de tolerancia angular
   * según el perfil biomecánico del usuario (Regla RD-02).
   * Flexibiliza hasta 20% si hay limitaciones de movilidad.
   */
  private calcularAjusteBiotipo(perfil: PerfilBiomecanico): number {
    ...
    // Detectar limitaciones: si algún rango es significativamente menor al estándar
    const rangoPromedioMax = rangos.reduce((sum, r) => sum + r.max, 0) / rangos.length;
    const RANGO_ESTANDAR_PROMEDIO = 140; // Grados promedio de flexión normal

    if (rangoPromedioMax < RANGO_ESTANDAR_PROMEDIO * 0.8) {
      return 20; // Máximo ajuste del 20%
  ...
  /**
   * Valida que la respuesta JSON de Gemini cumple el schema esperado.
   * Previene la instanciación de AnalisisBiomecanico con datos corruptos.
   * (Capítulo VII - Integridad: Validación de Esquema JSON)
   */
  validateResponse(jsonString: string): GeminiEvaluationResponse | null {
    try {
      // Limpiar posibles artefactos de formato (```json, etc.)
      ...
      if (
        ...
        console.error('PromptBuilder: puntuacionGeneral inválida');
        return null;
      }

      if (!Array.isArray(parsed.errores)) {
        console.error('PromptBuilder: errores no es un array');
        return null;
      }

      if (!parsed.recomendacionAdaptativa || !parsed.recomendacionAdaptativa.tipoEstrategia) {
        console.error('PromptBuilder: recomendacionAdaptativa faltante');
        return null;
      }
      ...
      for (const error of parsed.errores) {
        if (!error.articulacion || typeof error.severidad !== 'string') {
          console.error('PromptBuilder: error individual malformado');
          return null;
        }
      ...
      console.error('PromptBuilder: Error parseando respuesta JSON:', e);
      return null;
    }
  }
}
